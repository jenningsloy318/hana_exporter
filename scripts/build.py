...
################
#### hana exporter Variables
################

# Packaging variables
PACKAGE_NAME = "hana_exporter"
INSTALL_ROOT_DIR = "/usr/bin"
CONFIG_DIR = "/etc/hana_exporter"


SYSTEMD_SCRIPT = "scripts/hana_exporter.service"
SCRIPT_DIR = "/usr/lib/systemd/system/"
DEFAULT_CONFIG = "hana_exporter.yml"
...
CONFIGURATION_FILES = [
    CONFIG_DIR + '/hana_exporter.yml'
]

# META-PACKAGE VARIABLES
PACKAGE_URL = "https://github.com/jenningsloy318/hana_exporter"
MAINTAINER = "jenningsloy318@gmail.com"
VENDOR = "jenningsloy318"
DESCRIPTION = "hana exporter to monitor hana database."
...
def create_package_fs(build_root):
    ...
    dirs = [ INSTALL_ROOT_DIR[1:],SCRIPT_DIR[1:], CONFIG_DIR[1:] ]
    for d in dirs:
...
def package_scripts(build_root, config_only=False, windows=False):
    ...
    if config_only :
        ...
        shutil.copyfile(DEFAULT_CONFIG, os.path.join(build_root, "hana_exporter.yml"))
        os.chmod(os.path.join(build_root, "hana_exporter.yml"), 0o644)
    else:
        logging.info("Copying scripts and configuration to build directory")
        shutil.copyfile(SYSTEMD_SCRIPT, os.path.join(build_root, SCRIPT_DIR[1:], SYSTEMD_SCRIPT.split('/')[1]))
        os.chmod(os.path.join(build_root, SCRIPT_DIR[1:],SYSTEMD_SCRIPT.split('/')[1]), 0o644)
        shutil.copyfile(DEFAULT_CONFIG, os.path.join(build_root, CONFIG_DIR[1:], "hana_exporter.yml"))
        os.chmod(os.path.join(build_root, CONFIG_DIR[1:], "hana_exporter.yml"), 0o644)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='hana exporter build and packaging script.')
