	...
	for logModeRows.Next() {
		if err := logModeRows.Scan(&log_mode); err != nil {
			...
			if log_mode_value, ok := parseConfigString(log_mode.String); ok {
				ch <- prometheus.MustNewConstMetric(logModeSystemDesc, prometheus.GaugeValue, log_mode_value, BaseLabelValues...)
			}
		} else {
			ch <- prometheus.MustNewConstMetric(logModeSystemDesc, prometheus.GaugeValue, 0, BaseLabelValues...)
		}
	}
